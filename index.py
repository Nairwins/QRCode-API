...
async def generate_qr(
    ...
    border:          int   = Query(5, ge=0, le=20, description="Quiet zone size in modules around the QR code"),
...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500" width="500" height="500">
  <image href="data:image/png;base64,{b64}" width="500" height="500"/>
        ...
        return Response(
            ...
            headers={"Content-Disposition": f"{'attachment' if download else 'inline'}; filename=\"{filename}.svg\""}
        )

    return Response(
        content=png_bytes,
        media_type="image/png",
        headers={"Content-Disposition": f"{'attachment' if download else 'inline'}; filename=\"{filename}.png\""}
    )
