...
def is_in_logo_area(row, col, n, logo_radius):
    """Check if a module is within the logo area (center circle)"""
    center = n / 2
    distance = ((row - center) ** 2 + (col - center) ** 2) ** 0.5
    return distance < logo_radius
...
def load_logo(logo_url=None, logo_bytes=None):
...
def add_logo_to_qr(qr_img, logo_img, logo_size_percent):
    """Add logo to center of QR code with transparency support"""
    qr_width, qr_height = qr_img.size
    
    # Calculate logo size (percentage of QR code)
    logo_size_percent = max(10, min(40, logo_size_percent))  # Clamp between 10-40%
...
def build_qr_image(
    ...
    # Calculate logo area to clear dots
    logo_radius_modules = 0
    if logo_url or logo_bytes:
        logo_size_clamped = max(10, min(40, logo_size))
        logo_radius_modules = (n * logo_size_clamped / 100) / 2
    ...
    if grad:
        ...
        for row in range(n):
            for col in range(n):
                # Skip eye modules and logo area
                if matrix[row][col] and not is_eye_module(row - border, col - border, n - 2 * border):
                    if not is_in_logo_area(row, col, n, logo_radius_modules):
                        x = col * size
                        y = row * size
    ...
    if logo_url or logo_bytes:
        logo_img = load_logo(logo_url, logo_bytes)
        if logo_img:
            final_img = add_logo_to_qr(final_img, logo_img, logo_size)
